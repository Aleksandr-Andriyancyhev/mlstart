...
epochs = 5
optimizer = optim.Adam(pretrained_resnet18.parameters(), lr=0.001)
loss_function = nn.CrossEntropyLoss()
best_loss = 1000000
best_acc = 0
for epoch in range(epochs):
    
    train_loss, train_acc = train(pretrained_resnet18, train_loader_1, optimizer, loss_function, device)

    test_loss, test_acc   = evaluate(pretrained_resnet18, test_loader_1, loss_function, device)
    ...
    if test_loss < best_loss:
        best_loss = test_loss
        torch.save(pretrained_resnet18, "resnet18_best_loss.pth")
